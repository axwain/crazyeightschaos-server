...
namespace CrazyEights.Tests.Entities
{
    ...
    public class RoomTests
    {
        ...
        public void Room_ChangeOwner()
        {
            ...
            Assert.That(room.RoomOwner?.Name, Is.EqualTo("First Owner"), "Room owner is the one pass on init");
        ...
        [Test, Description("Removing last player resets the room id")]
        public void Room_RemovePlayer_LastPlayer()
        {
            ...
            Assert.That(room.Id, Is.EqualTo(-1), "Room should have the non-init id");
        }

        [Test, Description("Removing a player after removing the last one is not allowed")]
        public void Room_RemovePlayer_AfterLastOne()
        {
            ...
            Assert.That(
                ...
                Throws.TypeOf<NotInitializedException>(),
                "Room remove throws after trying to remove from an empty room"
            );
        }
        ...
        [Test, Description("Room must be initialized before any other operation is run")]
        public void Room_MustBeInit()
        {
            var owner = new Player { Id = 0, Name = "Owner" };
            var otherPlayer = new Player { Id = 1, Name = "Player1" };
            var maxPlayerCount = 5;
            ...
            Assert.That(
                () => room.Add(otherPlayer),
                Throws.TypeOf<NotInitializedException>(),
                "Room can't add player before init"
            );
            Assert.That(
                () => room.ChangeRoomOwner(owner.Id, otherPlayer.Id),
                Throws.TypeOf<NotInitializedException>(),
                "Room can't change owner before init"
            );
            Assert.That(
                () => room.Remove(owner.Id),
                Throws.TypeOf<NotInitializedException>(),
                "Room can't remove player before init"
            );
        }
    }
}